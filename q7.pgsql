...
create or replace function dotmap_generate_points(geom geometry, block_idx bigint, dotcount integer)
returns jsonb
language plpgsql
as $$
declare
...
begin
    while 1 > 0 loop
        ...
        SELECT count(block_idx) as n, jsonb_agg(block_idx) as block_idx, jsonb_agg((dp).path[1]-1) as dot_idx, jsonb_agg(ST_X((dp).geom)) as x, jsonb_agg(ST_Y((dp).geom)) as y
        INTO rec
        FROM (
             -- Points from blocks
            SELECT 
        ...
        if jsonb_array_length(rec.block_idx) = dotcount then
            return to_jsonb(rec);
        end if;
    end loop;
end;$$;
...
--explain analyze
select dotmap_generate_points(geom, indexes.idx, dotcounts.dotcount)
